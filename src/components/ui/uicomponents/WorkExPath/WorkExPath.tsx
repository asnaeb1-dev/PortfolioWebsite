...
const IndividualWorkExPath = ({ begin = false, isDone, onclick, index = 0, end = false, details }) => {
	...
	return (
		<div style={{ transform: `translateY(-${20 * index}px)` }} className={`flex flex-col relative items-center`}>
			...
			{ !end && <div onTransitionEnd={() => (setTransitionCompleted(true), isDone())} className={`w-1 ${start ? "h-36 md:h-52" : "h-0"} bg-indigo-500 rounded-full duration-[3000ms] transition-all`}></div>}
			{
				details?.companyName && 
					<WorkExpCard
