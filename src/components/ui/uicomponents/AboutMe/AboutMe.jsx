...
const AboutMe = ({ aboutMeText = "", closeAboutMe }) => {
  const boxRef = useRef();
  ...
  return (
    <div
      ref={boxRef}
      className="my-container shadow-xl w-full h-[330px] md:h-[400px] bg-indigo-500/10 backdrop-blur-md backdrop-saturate-200 lg:w-full lg:h-full overflow-hidden flex flex-col p-4 xl:p-8 rounded-3xl gap-2 md:gap-4 lg:gap-5"
    >
      ...
      <span className=" h-full flex-col w-full lg:gap-3 flex flex-1 md:flex-[0] gap-2">
        <p className="text-sm md:text-xl font-bold">What do I do?</p>
...
const SkillBlock = ({ type = "", text, dimension = 48, index = 0 }) => {
  ...
  return (
    ...
    >
      ...
      <p className="font-semibold text-center hidden md:block">{text}</p>
    </div>
  );
};
